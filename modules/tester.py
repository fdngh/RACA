...
class Tester(BaseTester):
    ...
    def test(self):
        ...
        self.model.eval()
        with torch.no_grad():
            test_gts, test_res, test_img_ids = [], [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, sentence_label) in tqdm(enumerate(self.test_dataloader)):
                images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device)
                _ = self.model(self.args, images, sentence_label, reports_ids, self.device, images_id, mode='analyze')
                output = self.model(self.args, images, sentence_label, reports_ids, self.device, images_id, mode='sample')
            
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
                
                # Store the image IDs
                test_img_ids.extend(images_id)
            ...
            # Create DataFrames with the image IDs
            results_df = pd.DataFrame({'image_id': test_img_ids, 'prediction': test_res})
            gts_df = pd.DataFrame({'image_id': test_img_ids, 'ground_truth': test_gts})
            
            # Save to CSV
            results_df.to_csv(os.path.join(self.save_dir, "res.csv"), index=False)
            gts_df.to_csv(os.path.join(self.save_dir, "gts.csv"), index=False)
        return log
    ...
    def plot(self):
        ...
        with torch.no_grad():
             for batch_idx, (images_id, images, reports_ids, reports_masks, sentence_label) in tqdm(enumerate(self.test_dataloader)):
                images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device)
                output = self.model(self.args, images, sentence_label, reports_ids, self.device,  mode='analyze')
